...
(* Advance an eventually/eventually_n goal by one backend step. The backend
   supplies its step relation, state type, conversion, and any processing
   needed after the new state has been introduced. *)
...
(* Propagate one decoded step through code-loading, assumptions, MAYCHANGE,
   and component reads. The normalization hook runs after decoding and before
   the common state-update machinery. *)
